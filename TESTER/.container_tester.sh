...
run_tests ()
{
	...
	clang++  -g -Werror -Wextra -Werror -std=c++98 -I . -I "$INCLUDES" -I "$INCLUDES"/EMPTY -I ../"$2" -I "$INCLUDES"/EMPTY/JUST_IN_CASE \
			mains/"$1"_main.cpp -o bin/"$1"_ft 2> logs/"$1"_ft.error.log
	clang++ -g -Werror -Wextra -Werror -std=c++98 -D STD -I . -I "$INCLUDES" -I "$INCLUDES"/EMPTY \
	...
	then
		...
		then
			...
			then
				if [[ "$OSTYPE" == "linux-gnu"* ]]
				then
					valgrind --leak-check=full ./bin/"$1"_ft > /dev/null 2> leak/"$1"_leak
